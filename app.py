...
def cadastrar_livro(livros):
    ...
    livros[titulo] = {"autor": autor, "paginas": paginas, "disponivel": True}
...
def buscar_livro(livros):
    ...
    for titulo, dados in livros.items():
        if termo in titulo.lower() or termo in dados["autor"].lower():
            status = "Disponível" if dados["disponivel"] else "Emprestado"
            print(f"- Título: {titulo} | Autor: {dados['autor']} | Status: {status}")
            encontrados = True
...
def registrar_emprestimo(alunos, livros, emprestimos):
    ...
    titulo = input("Título exato do livro: ").strip()
    ...
    print(f"Empréstimo do livro '{titulo}' registrado com sucesso para {alunos[cpf]['nome']}!")
...
def consultar_pendencias(alunos, emprestimos):
    ...
    for e in pendentes:
        ...
        print(f"Livro: '{e['titulo_livro']}' | Aluno: {nome_aluno} | Status: {status_prazo}")
